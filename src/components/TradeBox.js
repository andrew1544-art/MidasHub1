...
function KYCForm({ onComplete }) {
  ...
  if (profile?.kyc_status === 'pending') return (
    <div className="text-center py-4">
      ...
      <p className="text-xs text-white/40 mb-3">Being reviewed. You can still trade.</p>
  ...
  return (
    <div className="space-y-3">
      <div className="text-center mb-2"><span className="text-3xl">🛡️</span><div className="font-bold text-sm mt-1">Verify Your Identity</div><div className="text-[10px] text-white/30">Required for trade protection</div></div>
      <div className="p-2.5 rounded-lg bg-blue-500/10 border border-blue-500/20 text-[10px] text-blue-300 leading-relaxed">🔒 Details only visible to admins for dispute resolution.</div>
      ...
      <div className="grid grid-cols-2 gap-2">
        <div><input type="file" ref={idRef} accept="image/*" className="hidden" onChange={e => setIdPhoto(e.target.files[0])} /><button onClick={() => idRef.current?.click()} className="w-full p-3 rounded-xl bg-white/5 border border-dashed border-white/15 text-center"><div className="text-lg mb-0.5">{idPhoto ? '✅' : '📄'}</div><div className="text-[9px] text-white/30">{idPhoto ? 'ID attached' : 'Upload ID photo'}</div></button></div>
        <div><input type="file" ref={selfieRef} accept="image/*" className="hidden" onChange={e => setSelfie(e.target.files[0])} /><button onClick={() => selfieRef.current?.click()} className="w-full p-3 rounded-xl bg-white/5 border border-dashed border-white/15 text-center"><div className="text-lg mb-0.5">{selfie ? '✅' : '🤳'}</div><div className="text-[9px] text-white/30">{selfie ? 'Selfie attached' : 'Selfie with ID'}</div></button></div>
      </div>
      {error && <div className="text-xs text-red-400 bg-red-500/10 p-2 rounded-lg">{error}</div>}
      <button onClick={handleSubmit} disabled={saving} className="btn-primary w-full py-2.5 text-sm disabled:opacity-40">{saving ? '⏳ Submitting...' : '🛡️ Submit Verification'}</button>
    </div>
  );
}

// ===== START TRADE — FULL SCREEN MODAL =====
export function StartTradeButton({ conversationId, otherUserId, onTradeCreated }) {
  const { user, profile, showToast } = useStore();
  const [open, setOpen] = useState(false);
  const [step, setStep] = useState('intro'); // intro → kyc → details
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const insertData = { conversation_id: conversationId, seller_id: sellerId, buyer_id: buyerId, title: form.title.trim(), description: form.description.trim(), amount: parseFloat(form.amount), currency: form.currency, payment_method: form.paymentMethod.trim() };
      ...
      await supabase.from('trade_messages').insert({ trade_id: data.id, content: `🔒 Trade: "${form.title}" for ${form.currency} ${parseFloat(form.amount).toFixed(2)} (+ 2% escrow). Waiting for ${role === 'seller' ? 'buyer' : 'seller'} to accept.`, is_system: true });
      ...
      setOpen(false); setForm({ title: '', description: '', amount: '', currency: 'USD', paymentMethod: '', categoryId: '' }); setRole(null); setStep('intro');
  ...
  if (!open) return (
    <button onClick={() => { setOpen(true); setStep(profile?.kyc_status === 'verified' || profile?.kyc_status === 'pending' ? 'intro' : 'kyc'); }}
      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-green-500/10 border border-green-500/20 text-green-400 text-xs font-semibold hover:bg-green-500/15 transition">🔒 Trade</button>
  );
  ...
  return (
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && setOpen(false)}>
      <div className="modal-content max-w-md p-5 sm:p-6">
        ...
        {/* Step 1: Role + Brief description */}
        {step === 'intro' && (
          <div className="space-y-4">
            <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/20 text-xs text-yellow-300 leading-relaxed">
              ⚠️ <strong>Escrow Protection:</strong> MidasHub holds payment until buyer confirms delivery. <strong>2% fee</strong> for safety.
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <button onClick={() => setRole('seller')}
                  className={`p-4 rounded-xl border transition text-center ${role === 'seller' ? 'bg-green-500/10 border-green-500/30' : 'bg-white/3 border-white/10 hover:border-green-500/20'}`}>
                  <div className="text-3xl mb-1.5">🏪</div>
                  <div className="text-sm font-bold">I&apos;m Selling</div>
                  <div className="text-[10px] text-white/30 mt-0.5">I have something to offer</div>
                </button>
                <button onClick={() => setRole('buyer')}
                  className={`p-4 rounded-xl border transition text-center ${role === 'buyer' ? 'bg-blue-500/10 border-blue-500/30' : 'bg-white/3 border-white/10 hover:border-blue-500/20'}`}>
                  <div className="text-3xl mb-1.5">🛒</div>
            ...
            <div>
              <div className="text-sm font-semibold mb-1.5">What do you want to trade?</div>
              <input value={form.title} onChange={e => setForm({...form, title: e.target.value})}
                placeholder="e.g., iPhone 15 Pro, Logo Design, Gaming Account..."
                className="input-field text-sm" style={{ fontSize: '16px' }} />
            </div>

            <div>
              <div className="text-sm font-semibold mb-1.5">Brief description</div>
              <textarea value={form.description} onChange={e => setForm({...form, description: e.target.value})}
                placeholder="Describe condition, what's included, delivery method..."
                className="input-field text-sm resize-none h-20" style={{ fontSize: '16px' }} />
            </div>

            {(!role || !form.title.trim()) && <p className="text-[10px] text-white/20 text-center">Select your role and describe what you&apos;re trading to continue</p>}

            <button onClick={() => { if (!role) { setError('Pick your role first'); return; } if (!form.title.trim()) { setError('Describe what you\'re trading'); return; } setError(''); setStep('details'); }}
              disabled={!role || !form.title.trim()}
              className="btn-primary w-full py-3 text-sm disabled:opacity-30">
              Continue → Set Price & Payment
            </button>
            {error && <div className="text-xs text-red-400 text-center">{error}</div>}
          </div>
        )}

        {/* Step 2: Price, currency, payment */}
        {step === 'details' && (
          <div className="space-y-3">
            <div className="flex items-center gap-2 text-sm mb-1">
              ...
              <button onClick={() => setStep('intro')} className="text-[var(--accent)] text-xs shrink-0">← Edit</button>
            </div>

            {categories.length > 0 && (
              <div>
                <div className="text-xs text-white/40 mb-1.5">Category</div>
            ...
            {form.amount && parseFloat(form.amount) > 0 && (
              <div className="p-3 rounded-lg bg-white/3 text-sm space-y-1.5">
                ...
                <div className="flex justify-between font-bold text-sm border-t border-white/5 pt-1.5"><span>Buyer total</span><span className="text-[var(--accent)]">{form.currency} {(parseFloat(form.amount) * 1.02).toFixed(2)}</span></div>
              </div>
            )}

            <input value={form.paymentMethod} onChange={e => setForm({...form, paymentMethod: e.target.value})}
              placeholder="Payment: Bank, PayPal, M-Pesa, Crypto..." className="input-field text-sm" style={{ fontSize: '16px' }} />

            {error && <div className="text-xs text-red-400 bg-red-500/10 p-2 rounded-lg">{error}</div>}

            <button onClick={handleCreate} disabled={creating || !form.amount}
...
export function TradeCard({ trade, onUpdate }) {
  ...
  const updateStatus = async (newStatus) => {
    ...
    try {
      ...
      const msgs = { accepted:'accepted the trade 🤝', paid:'marked payment as sent 💰', delivered:'marked as delivered 📦', completed:'confirmed receipt 🎉', disputed:'raised a dispute ⚠️', cancelled:'cancelled the trade' };
    ...
    <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${config.color}30`, background: `${config.color}05` }}>
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-3">
          ...
          <div className="p-2 rounded-lg bg-yellow-500/8 border border-yellow-500/15 text-[10px] text-yellow-300/70">🔒 Escrow protected. Never share banking details in chat.</div>
          {isActive && (
